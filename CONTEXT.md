...
ANIM-2 ROOT CAUSE FOUND + FIXED: Marina never saw any page transition (morph OR fly-in) on real clicks. Cause: the nav pills are <a href=#frame-x> but the page updates the hash via history.replaceState, which does NOT fire a hashchange event. Both the morph and the fly-in listened only for hashchange, so they never ran on genuine navigation. They only fired in automated tests because setting location.hash directly DOES fire hashchange - a misleading test path. Fix: replaced the hashchange listener in mk-fly-js with a MutationObserver watching the .frame elements for the .is-active class moving to a new frame; runs the fly-in on the newly-active frame. Verified firing (3 anims) on a REAL nav-link click on the committed live page across Work/About/Contact. Current committed page transition = fly-in (panes rise from below, fade+scale in, staggered by column, 620ms, no glow). mockup.html now 106725 chars.

ANIM-2 FINAL: Swapped the page transition back to the MORPH (Marina preferred it) now that the MutationObserver trigger works. The morph never actually fired on real clicks before for the same replaceState/hashchange reason as the fly-in. Rebuilt the FLIP morph (incoming panes animate from previous frame nearest-pane geometry: translate+scale, opacity 0.65->1, 800ms cubic-bezier(0.5,0,0.2,1)) on the SAME MutationObserver-on-.is-active trigger; caches prev frame rects on load and after each settle so there is a real delta. Verified firing (3 anims) on a REAL nav click on the committed live page. Current committed page transition = morph. mockup.html now 107488 chars.

## ANIM-2 page morph reworked to true FLIP from previous card position (committed)
Marina: cards should look like the SAME framed cards resizing/repositioning to fit each page's content, morphing FROM the previous frame's card location (not fading up in place). Hero transitions are Home<->About<->Contact (all 3-column framed-card layouts); Work (single full-width shelf) gets best-effort. Implemented in mockup.html mk-fly-js block: one-to-one greedy matching (each incoming card claims its closest UNUSED previous card; cost=center distance + 0.25*size diff), so each card animates independently FROM its matched previous card's actual rect (translate from old top-left + scale from old size, transformOrigin top-left, opacity 0.85->1, cubic-bezier(0.45,0,0.15,1), 700ms). Unmatched cards (e.g. About's 3rd card when coming from 2-card Contact) soft fade/scale-in (0.92->1). Still on MutationObserver-on-.is-active trigger (NOT hashchange) + reduced-motion gated. Note: inactive frames have zero-size rects, so prevRects are cached while the outgoing frame is still active. Match maps verified sensible (Home->About center->center/right->right with a left<->center crossover that is geometrically honest since About shifts rightward). Verified on committed live page with REAL clicks: Home->About fired 3 card anims, About->Contact fired 2. mockup.html now 108188 chars.

## ANIM-3 in-panel tab transition (committed, DONE) — all 3 animations complete
